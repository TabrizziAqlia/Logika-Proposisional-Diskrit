...
function cekMatematikaDiskritLanjutan(p, c) {
    return p && c; // Konjungsi (AND): L ↔ (P ∧ C)
}

function cekStrukturData(d, a) {
    return d || a; // Disjungsi Inklusif (OR): S ↔ (D ∨ A)
}
...
document.addEventListener('DOMContentLoaded', () => {
    const mainContainer = document.getElementById('logika-container');
    ...
    // Masukkan Studi Kasus ke wadah yang sudah ada
    ...
    // Render MathJax untuk menampilkan notasi matematika dengan benar
    if (window.MathJax) {
